...
template <typename T, int Dim,
          typename IndexT, template <typename U> class PtrTraits>
void
THCDeviceTensor<T, Dim, IndexT, PtrTraits>::fillAsync(T val,
                                                      cudaStream_t stream) {
  // for now, don't parameterize the union integer type based on `T`,
  // we only deal with floats at the moment. Change when we support
  // more than `float`.
  thc_static_assert(sizeof(T) == sizeof(int));
#ifdef __CUDA_ARCH__
  assert(isContiguous());
#else
  if (!isContiguous()) {
